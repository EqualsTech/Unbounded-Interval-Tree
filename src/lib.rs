    ...
    Q: Ord + Clone,
{
    ...
    pub fn get_interval_overlaps(&self, q: &Range<Q>) -> Vec<&Range<Q>> {
        ...
        Self::get_interval_overlaps_looped(curr, q, &mut acc);
        acc
    }
    ...
    fn get_interval_overlaps_looped<'a>(
        curr: &'a Option<Box<Node<Q>>>,
        q: &Range<Q>,
        ...
        loop {
            'inner: loop {
                ...
                match (max_subtree, min_q) {
                    (Some(max_subtree), Some(min_q)) if max_subtree < min_q => break 'inner,
                    _ => {
            ...
            match stack.pop_back() {
                Some(popped) => {
                    ...
                    match (min_node, max_q) {
                        (Some(min_node), Some(max_q)) if min_node > max_q => {
                            if stack.len() == 0 {
                                return;
                            }
                            continue;
